...
static void btif_gatt_dm_update_properties(btif_gatt_dm_cb_t *p_btif_cb)
{
    ...
    if(p_eir_remote_name)
    {
        ...
        btif_dm_update_ble_remote_properties(p_btif_cb->bd_addr.address,   bdname.name,p_btif_cb->device_type);
        #endif
    }
}
...
static void btgatt_dm_handle_event(uint16_t event, char *p_param)
{
    ...
    switch(event)
    {
        ...
        case BTIF_GATT_DM_SET_ADV_EXT_PARAM:
        {
            ...
            BTA_DmSetBleAdvExtParams(p_adv_ext_param->adv_int_min, p_adv_ext_param->adv_int_max, p_adv_ext_param->adv_type, p_adv_ext_param->own_addr_type,
                p_adv_ext_param->chnl_map, p_adv_ext_param->afp, p_adv_ext_param->peer_addr_type, p_adv_ext_param->dir_addr);
            break;
        }
        ...
        case BTIF_GATT_DM_SET_ADV_ENABLE:
        {
            ...
            BTA_DmSetVisibility(p_visibility->disc|BTA_DM_IGNORE, p_visibility->conn|BTA_DM_IGNORE, BTA_DM_IGNORE, BTA_DM_IGNORE);
            break;
        }
...
#define BLE_ISVALID_PARAM(x, min, max)  ((x) >= (min) && (x) <= (max))
    ...
    if(!BLE_ISVALID_PARAM(param->adv_int_min, 0x20, 0x4000) || !BLE_ISVALID_PARAM(param->adv_int_max, 0x20, 0x4000))
    {
...
tls_bt_status_t tls_ble_scan(bool start)
{
    ...
    return btif_transfer_context(btgatt_dm_handle_event, start ? BTIF_GATT_DM_SCAN_START : BTIF_GATT_DM_SCAN_STOP,
                             (char *) NULL, 0, NULL);
}
...
tls_bt_status_t tls_dm_start_timer(uint8_t id, uint32_t timeout_ms, tls_ble_dm_timer_callback_t callback_ptr)
{
	CHECK_BTGATT_INIT();
