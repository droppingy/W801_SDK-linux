...
#define WM_BT_STORAGE_DEBUG_TAG 1
...
bool btif_wm_config_find_by_key(BD_ADDR bd_addr, int *index, int add)
{
    ...
    if((found == false) && (add))
    {
        ...
        *index = (BTM_SEC_MAX_DEVICE_RECORDS - 1);
        ...
        tls_param_set(TLS_PARAM_ID_BT_REMOTE_DEVICE_1 + (BTM_SEC_MAX_DEVICE_RECORDS - 1), (void *)&device, 0);
...
int btif_wm_config_get_remote_int(const nv_tag_t section, const char *key, const nv_tag_t name, int *value)
{
    int size = sizeof(*value);
    int type = BTIF_CFG_TYPE_INT;
    return btif_wm_config_get_remote(section, key, name, (char *)value, &size, &type, 0);
}
int btif_wm_config_set_remote_int(const nv_tag_t section, const char *key, const nv_tag_t name, int value)
{
    return btif_wm_config_set_remote(section, key, name, (char *)&value, sizeof(value), BTIF_CFG_TYPE_INT, false);
}
int btif_wm_config_get_remote_str(const nv_tag_t section, const char *key, const nv_tag_t name, char *value, int *size)
{
    int type = BTIF_CFG_TYPE_STR;
...
int btif_wm_config_set_remote_str(const nv_tag_t section, const char *key, const nv_tag_t name, const char *value)
{
    value = value ? value : "";
    return btif_wm_config_set_remote(section, key, name, value, strlen(value) + 1, BTIF_CFG_TYPE_STR, false);
}
...
int btif_config_set_int(const char *section, const char *key, const char *name, int value)
{
    ...
    else
    {
        if(strncmp(section, "Remote", 6) == 0)
        {
            ...
            if(index > 0)
            {
                return btif_wm_config_set_remote(NV_REMOTE, key, index, (char *)&value, sizeof(value), BTIF_CFG_TYPE_INT, false);
            }
        }
    }
...
int btif_config_set_str(const char *section, const char *key, const char *name, const char *value)
{
    ...
    else
    {
        if(strncmp(section, "Remote", 6) == 0)
        {
            ...
            if(index > 0)
            {
                return btif_wm_config_set_remote(NV_REMOTE, key, index, value, strlen(value) + 1, BTIF_CFG_TYPE_STR, false);
            }
        }
    }
