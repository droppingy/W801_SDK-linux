...
static void btspp_data_indication_callback(uint8_t status, uint32_t handle, uint8_t *p_data, uint16_t length)
{
    int i = 0;
    TLS_BT_APPL_TRACE_DEBUG("%s, status=%d\r\n", __FUNCTION__, status);  
#if 0   
...
static void btspp_server_start_callback(uint8_t status, uint32_t handle, uint8_t sec_id, bool use_co_rfc)
{
...
static void wm_bt_spp_callback(tls_spp_event_t evt, tls_spp_msg_t *msg)
{
	...
	switch(evt)
	{
        ...
        case WM_SPP_START_EVT:
            btspp_server_start_callback(msg->start_msg.status, msg->start_msg.handle, msg->start_msg.sec_id, msg->start_msg.use_co_rfc);
            break;
        case WM_SPP_CL_INIT_EVT:
            break;
        case WM_SPP_DATA_IND_EVT:
            btspp_data_indication_callback(msg->data_ind_msg.status, msg->data_ind_msg.handle, msg->data_ind_msg.data, msg->data_ind_msg.length);
            break;
