...
void ble_client_demo_api_connect_callback(int conn_id, int status, int client_if, tls_bt_addr_t *bda)
{
...
void ble_client_demo_api_search_service_result_callback(int conn_id, tls_bt_uuid_t *p_uuid, uint8_t inst_id)
{
    TLS_BT_APPL_TRACE_DEBUG("%s, conn_id=%d\r\n", __FUNCTION__, conn_id);
}
...
void ble_client_demo_api_notify_callback(int conn_id, uint8_t *value, tls_bt_addr_t *addr, uint16_t handle, uint16_t len, uint8_t is_notify)
{
    g_recv_size += len;
    g_recv_count++;
    if(g_recv_count > 100)
...
void ble_client_demo_api_get_gatt_db_callback(int status, int conn_id, tls_btgatt_db_element_t *db, int count)
{
    int i = 0;
...
void ble_client_demo_api_services_removed_callback(int conn_id, uint16_t start_handle, uint16_t end_handle)
{
    TLS_BT_APPL_TRACE_DEBUG("%s, conn_id=%d\r\n", __FUNCTION__, conn_id);
}

/** GATT services were added */
void ble_client_demo_api_services_added_callback(int conn_id, tls_btgatt_db_element_t *added, int added_count)
{
    TLS_BT_APPL_TRACE_DEBUG("%s, conn_id=%d\r\n", __FUNCTION__, conn_id);
}
...
static bool analyse_adv_data(tls_bt_addr_t *addr, int rssi, uint8_t *adv_data)
{
    ...
    while(index < 62)
    {
        ...
        if(type == 0x09)
        {
            if((adv_data[index++] == 'W') && (adv_data[index++] == 'M') && (adv_data[index++] == 'B')&& (adv_data[index++] == 'L')&& (adv_data[index++] == 'E'))
            {
                found = true;
