...
static gattArray_t gatt_uuid[] =
{
    ...
    {0,     0xFFF2, ATTR_CHARACTISTIRC, 0, 0x08, 0x11},
...
static void ble_server_connection_cb(int conn_id, int server_if, int connected, tls_bt_addr_t *bda, uint16_t reason)
{
    g_conn_id = conn_id;
...
static void ble_demo_server_notification_started(int id)
{
    ...
    ss++;
    if(ss > 0xFE) ss = 0x00;
	tls_ble_server_send_indication(g_server_if,gatt_uuid[DEMO_KEY_VALUE_INDEX].attr_handle,g_conn_id,g_mtu,1,g_ind_data);
...
static void ble_server_indication_sent_cb(int conn_id, int status)
{
    ...
    if(g_ble_output_fptr == NULL)
    {
    	...
    	tls_ble_server_send_indication(g_server_if,gatt_uuid[DEMO_KEY_VALUE_INDEX].attr_handle,conn_id,g_mtu,1,g_ind_data); 
